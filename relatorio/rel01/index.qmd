...
O presente relatório aborda a aplicação da programação e da Estatística Descritiva no contexto da Engenharia Civil, com foco na análise da resistência à compressão do concreto. Inicialmente, são apresentados os fundamentos da linguagem R, com base no curso R Básico 2024, destacando recursos de programação e manipulação de dados relevantes para a análise estatística.

Em seguida, são apresentados os principais conceitos de Estatística Descritiva, tendo como fundamentação os quatro capítulos iniciais do livro Estatística & Probabilidade Aplicada às Engenharias e Ciências, de Ben Dêivide de O. Batista. A partir dessa referência, são abordadas medidas de posição e dispersão, bem como conceitos relacionados à assimetria e à curtose, estabelecendo a base teórica necessária para a interpretação dos dados.

Por fim, os conceitos estudados são aplicados a dados provenientes de ensaios de resistência à compressão do concreto, utilizando o pacote leem para a organização, tabulação e representação gráfica dos dados, além do cálculo das principais medidas estatísticas. Os resultados são analisados e discutidos de forma crítica, relacionando os procedimentos computacionais aos fundamentos matemáticos e estatísticos e à sua aplicação prática na Engenharia Civil.
...
Nesta seção, aplicamos o pacote `leem`, desenvolvido pelo Prof. Ben Dêivide, para realizar a análise estatística de 30 séries de concreto empregadas na execução de estacas de fundação de um empreendimento em **Conselheiro Lafaiete – MG**, sob responsabilidade da empresa **Mount Engenharia Ltda.**
