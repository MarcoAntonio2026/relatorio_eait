...
Para obter uma visão geral das variáveis presentes na base de dados, foi utilizada a função `summary()`, que apresenta informações diferentes de acordo com o tipo de variável.

```{r}
summary(dados)
```

A função permite identificar a quantidade de observações, o número de valores distintos, as principais medidas estatísticas e as categorias das variáveis. A tabela abaixo apresenta um resumo dos resultados obtidos nessa função:

| Informação apresentada | Tipo de variável | Significado                                           |
| ---------------------- | ---------------- | ----------------------------------------------------- |
| **Length**             | Quantitativa     | Quantidade total de elementos da variável.            |
...
| **Categorias**         | Qualitativa      | Categorias existentes e suas respectivas frequências. |
