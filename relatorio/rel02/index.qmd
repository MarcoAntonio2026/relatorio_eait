...
Foi criada a variável `acima_media` para indicar se a resistência de cada corpo de prova está acima da **resistência média do seu respectivo tipo de concreto**.

Inicialmente, foi calculada a resistência média para cada classe utilizando `ave()`. Em seguida, cada resistência observada foi comparada com a média correspondente.

```{r}
# Cálculo da resistência média por tipo de concreto
media_tipo <- ave(
...
# Classificação acima ou abaixo da média
dados_sem_na$acima_media <- 
  dados_sem_na$resistencia_mpa > media_tipo

# Visualização dos resultados
dados_sem_na[, c("tipo_concreto", "resistencia_mpa", "acima_media")]
```

A variável `acima_media` assume o valor **`TRUE`** quando a resistência do corpo de prova é superior à média de seu respectivo tipo de concreto e **`FALSE`** quando é igual ou inferior à média.

Dessa forma, a comparação é realizada dentro de cada classe, evitando que corpos de prova de diferentes tipos de concreto sejam comparados com uma única média geral.
