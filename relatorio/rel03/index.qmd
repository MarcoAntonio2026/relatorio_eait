...
Para verificar, em termos descritivos, se os concretos utilizados no Bloco C apresentam desempenho superior aos demais blocos, foram calculadas medidas de posição e dispersão da resistência à compressão por obra.
...
Para complementar a comparação, foi utilizado um gráfico de caixa:
...
A comparação das médias e medianas permite identificar qual obra apresenta os maiores valores típicos de resistência, enquanto o `boxplot` permite avaliar também a dispersão dos resultados e possíveis valores discrepantes.

Assim, **se o Bloco C apresentar média e mediana superiores às demais obras, com distribuição dos resultados também predominantemente mais elevada, os dados fornecem suporte descritivo à afirmação do engenheiro**. Caso exista grande sobreposição entre as distribuições ou elevada variabilidade, a afirmação deve ser considerada com cautela.

Essa análise é **descritiva**, portanto permite comparar o desempenho observado na amostra, mas não permite afirmar que o Bloco C possui desempenho estatisticamente superior aos demais.
